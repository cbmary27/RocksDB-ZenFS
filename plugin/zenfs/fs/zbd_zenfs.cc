...
void ZonedBlockDevice::LogGarbageInfo() {
  ...
  for (auto z : io_zones) {
    ...
    if (z->IsFull()) {
      if (z -> is_finished) //defined in zbd_zenfs.h 
      {
        //garbage_bytes = z->effective_wp - z->max_capacity_ - z->used_capacity_;
        
        //c: 1. if we are including just the invalid data
        //effective_wp is set in Finish()
        garbage_bytes = z->effective_wp - z->start_ - z->used_capacity_;

        //c: 2. If we want to include the dummy writes as garbage as well
        //garbage_bytes = z->max_capacity_ - z->used_capacity_;
      }
      else
      {
        //zone not finished, so are we just counting the invalid data?
        garbage_bytes = z->max_capacity_ - z->used_capacity_;
      }
    }
    else
    {
      garbage_bytes = z->wp_ - z->start_ - z->used_capacity_;
    }

    //c: this is accumulating the total garbage bytes across all the zones
    total_garbage_bytes += garbage_bytes;
  ...
  avg_garbage_ += total_garbage_bytes;
  avg_garbage_count_++;

  uint64_t current_avg = avg_garbage_ / avg_garbage_count_;

  Info(logger_, "Total Garbage Bytes this interval: %lu bytes", total_garbage_bytes); 
  Info(logger_, "Cumulative Average Garbage Bytes (over %lu intervals): %lu bytes",
     avg_garbage_count_, current_avg);
  ...
  //c: logging
  Info(logger_, "Total Garbage Bytes: %lu bytes", total_garbage_bytes);
}
...
IOStatus ZonedBlockDevice::ApplyFinishThreshold() {
  IOStatus s;
  //checking if applyfinishthreshold is called
  Info(logger_, "Entering Apply Finish Threshold Function");
  ...
  for (const auto z : io_zones) {
    if (z->Acquire()) {
      ...
      if (!(z->IsEmpty() || z->IsFull()) && within_finish_threshold) {
        ...
        Info(logger_, "Finishing zone: Entering Finish()");
        s = z->Finish();
        if (!s.ok()) {
          z->Release();
